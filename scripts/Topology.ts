...
class Topology {
    private readonly offSetX: number; // game fields off set x
    private readonly offSetY: number; // game fields off set y
    private secret: boolean; // if true => that instance ships will be visible in the game
    private shipSet: ShipSet; // ship set instance
    private checkSet: CheckSet; // check set instance
    public readonly entity:Entity; // entity type

    constructor(entity:Entity, offSetX:number, offSetY:number, secret:boolean) {
        this.entity = entity;
     ...
     * @param point
     */
    public isPointUnder(point:Point):boolean {
     ...
     * @param point
     */
    public getCoordinats(point:Point): Check {
        return (!this.isPointUnder(point)) ? null :
            new Check(
                Math.floor(Math.max(0, Math.min(9, ((point.x - this.offSetX) - game.FIELD_SIZE) / game.FIELD_SIZE))),
                Math.floor(Math.max(0, Math.min(9, ((point.y - this.offSetY) - game.FIELD_SIZE) / game.FIELD_SIZE))),
                CheckType.CHECKED
            );
    }
     ...
     * @param ship
     */
    public canStay(ship: Ship): boolean {
        ...
        for (const ship of this.shipSet.getShips()) {
            if (ship.direct === 0) {
                for (let x:number = ship.x - 1; x < ship.x + ship.size + 1; x++) {
                    for (let y:number = ship.y - 1; y < ship.y + 2; y++) {
                        if (map[y] && map[y][x]) {
                            map[y][x] = false;
                        }
                    }
                ...
                for (let x:number = ship.x - 1; x < ship.x + 2; x++) {
                    for (let y: number = ship.y - 1; y < ship.y + ship.size + 1; y++) {
                        if (map[y] && map[y][x]) {
                            map[y][x] = false;
                        }
                    }
     ...
     * @param context
     */
    public draw(context:CanvasRenderingContext2D):Topology {
     ...
     * @param context
     */
    private drawFields(context:CanvasRenderingContext2D):Topology {
        context.lineWidth = game.FIELD_SIZE * 1.7 / 30;
        context.strokeStyle = game.getViewMode() === ViewMode.DARK ? '#f4f3f3' : 'blue';
        for (let i = 1; i <= 11; i++) {
            context.beginPath();
            context.moveTo(
                this.offSetX + i * game.FIELD_SIZE,
                this.offSetY
            );
            context.lineTo(
                this.offSetX + i * game.FIELD_SIZE,
                this.offSetY + 11 * game.FIELD_SIZE
            );
            context.stroke();
        }
        ...
        for (let i = 1; i <= 11; i++) {
            ...
            context.moveTo(
                this.offSetX,
                this.offSetY + i * game.FIELD_SIZE
            );
            context.lineTo(
                this.offSetX + 11 * game.FIELD_SIZE,
                this.offSetY + i * game.FIELD_SIZE
            );
            context.stroke();
        }
        ...
        for (let i = 0; i < 10; i++) {
            ...
            context.fillText(
                letter,
                this.offSetX + i * game.FIELD_SIZE + 1.5 * game.FIELD_SIZE,
                this.offSetY + game.FIELD_SIZE * 0.7
            );
        }
        ...
        for (let i = 0; i <= 9; i++) {
            context.fillText(
                String(i + 1),
                this.offSetX + game.FIELD_SIZE * 0.5,
                this.offSetY + i * game.FIELD_SIZE + 1.7 * game.FIELD_SIZE
            );
        }
        return this;
    }
     ...
     * @param ship
     */
    public drawShip(context:CanvasRenderingContext2D, ship:Ship):Topology {
        context.fillStyle = game.getViewMode() === ViewMode.LIGHT ? 'rgba(0,0,0,0.75)' : 'rgba(255,255,255,0.75)';
        context.beginPath();
        context.rect(
            this.offSetX + ship.x * game.FIELD_SIZE + game.FIELD_SIZE + 2,
            this.offSetY + ship.y * game.FIELD_SIZE + game.FIELD_SIZE + 2,
            (ship.direct === 0 ? ship.size : 1) * game.FIELD_SIZE - 4,
            (ship.direct === 1 ? ship.size : 1) * game.FIELD_SIZE - 4
        );
        context.fill();
        return this;
    }
     ...
     * @param check
     */
    private drawCheck(context, check:Check):Topology {
        ...
        if (check.type === CheckType.CHECKED) {
            ...
            context.beginPath();
            context.arc(
                this.offSetX + check.x * game.FIELD_SIZE + game.FIELD_SIZE * 1.5,
                this.offSetY + check.y * game.FIELD_SIZE + game.FIELD_SIZE * 1.5,
                game.FIELD_SIZE * 3 / 30,
                0,
                Math.PI * 2
            );
            context.fill();
     ...
     * @param injury
     */
    private drawInjury(context:CanvasRenderingContext2D, injury:Check): Topology {
        context.strokeStyle = 'red';
        context.lineWidth = game.FIELD_SIZE * 1.5 / 30;
        context.beginPath();

        context.moveTo(
            this.offSetX + injury.x * game.FIELD_SIZE + game.FIELD_SIZE,
            this.offSetY + injury.y * game.FIELD_SIZE + game.FIELD_SIZE
        );
        context.lineTo(
            this.offSetX + injury.x * game.FIELD_SIZE + game.FIELD_SIZE * 2,
            this.offSetY + injury.y * game.FIELD_SIZE + game.FIELD_SIZE * 2
        );
        context.stroke();
     ...
     * @param killed
     */
    private drawKilled(context:CanvasRenderingContext2D, killed:Check):Topology {
        ...
        context.moveTo(
            this.offSetX + killed.x * game.FIELD_SIZE + game.FIELD_SIZE,
            this.offSetY + killed.y * game.FIELD_SIZE + game.FIELD_SIZE
        );
        context.lineTo(
            this.offSetX + killed.x * game.FIELD_SIZE + game.FIELD_SIZE * 2,
            this.offSetY + killed.y * game.FIELD_SIZE + game.FIELD_SIZE * 2
        );
        context.stroke();
     ...
     * @param ships
     */
    public setShipSet(ships:ShipSet):Topology {
        this.shipSet = ships;
        return this;
    }
     ...
     * @param checks
     */
    public setCheckSet(checks:CheckSet):Topology {
     ...
     * @param ships
     */
    public addShips(...ships: Ship[]): Topology {
     ...
     * @param checks
     */
    public addChecks(...checks: Check[]): Topology {
     ...
     * @param secret
     */
    public setSecret(secret:boolean):Topology {
        this.secret = secret;
        return this;
    }
